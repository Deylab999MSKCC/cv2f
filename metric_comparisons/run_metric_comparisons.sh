...
then
cmd="Rscript run_cv2f_MAF5LDmatch_metrics_comparisons.R --positive_set_train $snpcell/$pos_train_file --negative_set_train $snpcell/$neg_train_file --positive_set_test $snpcell/$pos_test_file --negative_set_test $snpcell/$neg_test_file --feature_file $featurecell/$feature_file --mafpath $mafpath  --bimpath $bimpath --ldblockspath $ldblockspath --output_metrics $outputcell/$metric_file"
bsub -W 360 -R "rusage[mem=10]" -e ../results/comparisons_metric/mwe_vs_mwe_baseline.metrics.cv2f.err -o ../results/comparisons_metric/mwe_vs_mwe_baseline.metrics.cv2f.out -n 1 "$cmd"
fi
