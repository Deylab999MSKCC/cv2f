...
for chrm in {1..22}; do 
	...
	score_file="mwe_baseline.${chrm}.cv2f"
	...
	bsub -W 360 -R "rusage[mem=10]" -e ../results/score/mwe_baseline.${chrm}.cv2f.err -o ../results/score/mwe_baseline.${chrm}.cv2f.out -n 1 "$cmd"
	fi
done
